	...
	<div class="container">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Chỉnh sửa <b>Người Dùng</b></h2>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-sm-6">
                        ...
                        <select name="role" class="form-select" required>
                            <c:forEach items="${ListR}" var="r">
                            	<c:if test="${user.getRole().getRoleId() == r.roleId}">
                            		<option value="${r.roleId}" selected>${r.roleName}</option>
                            	</c:if>
                            	<c:if test="${user.getRole().getRoleId() != r.roleId}">
                            		 <option value="${r.roleId}" ${user.getUserName() == loginedUser.getUserName()?'disabled':''}>${r.roleName}</option>
                            	</c:if>
                            </c:forEach>
                        </select>
                        <hr/>
